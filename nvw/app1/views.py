...
'''
        ...
        output_zip.writestr('topology.json', subprocess.run(
            os.environ['_NVIEW_TOPOLOGY_COMMAND'], capture_output=True, shell=True,
...
settings_dir = str(pathlib.Path.home() / '.nvw')
sys.path.append(settings_dir)
import settings as nvw_settings
device_agents = nvw_settings.device_agents
...
def get_node_info(device_name, env_name):
    ...
    output_str = subprocess.run(
        os.environ['_NVIEW_TOPOLOGY_COMMAND'], capture_output=True, shell=True, text=True, cwd=f'{envdir}/{env_name}').stdout
